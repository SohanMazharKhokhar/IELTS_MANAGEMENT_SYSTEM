...
const ExercisesManagement: React.FC<ModuleListProps> = ({ moduleType, onEdit }) => {
  ...
  const filteredExercises = allExercises.filter(ex => ex.exerciseType === moduleType);
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredExercises.length > 0 ? filteredExercises.map(ex => (
              <tr key={ex.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{ex.tasks.length}</td>
              ...
              <tr>
                <td colSpan={4} className="text-center py-10 text-gray-500">
                  No {moduleType} exercises found. Click 'Create New Exercise' to add one.
                </td>
              </tr>
            )}
          </tbody>
